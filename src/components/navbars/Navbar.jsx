import { Bell, Home, Menu, Mic, Plus, Search, History, ListVideo, Settings, MessageCircleQuestionMark, MessageSquareWarning, Palette, SquarePen, Radio, X, ArrowLeft } from 'lucide-react'
...
function Navbar() {
  ...
  // ------ NAYA FUNCTION: Search Execute Karne Ke Liye ------
  const executeSearch = (query) => {
    if (query && query.trim().length > 0) {
      setSearchHistory(false); // Search hote hi dropdown band kar do
      navigate(`/?search_query=${encodeURIComponent(query)}`); // URL update
    }
  }

  const menuList = [
    { icon: <Home />, name: "Home", function: () => navigate("/") }, // Home click par wapas Home laye
    { icon: <History />, name: "History" },
    { icon: <ListVideo />, name: "Playlist" },
    { icon: <YoutubeSVG />, name: "Your videos" }, 
    ...
    <>
      ...
      <div
        ...
        <div className=' flex gap-2 items-center '>
          ...
          {/* Logo click hone par Home page par le jaye */}
          <svg
            onClick={() => navigate("/")}
        ...
        <div className=' hidden md:flex gap-5'>
          <div
            ...
            <input
              ...
              onKeyDown={(e) => { // <-- ENTER key dabane par search hoga
                if (e.key === 'Enter') {
            ...
            <Search
              onClick={(e) => { // <-- Search Icon click karne par search hoga
                e.stopPropagation();
                executeSearch(searchValue);
              }}
            ...
            >
              {searchHistoryData.map((data, i) => (
                  <div key={i}
                    onClick={(e) => { // <-- History item click karne par search hoga
                      e.stopPropagation();
                      setSearchValue(data);
                      executeSearch(data);
                    }}
                    className={` flex items-center gap-3 p-2 py-2   
                      ...
                      `}
                  ><History /> <span className='w-full overflow-hidden'>{data}</span><X onClick={(e) => e.stopPropagation()} /></div>
                ))
              }
            </div>
          </div>
        ...
        <div className=' flex gap-4  items-center  '>
          <div className=' flex md:hidden  '>
            ...
            >
                  ...
                  `}>
                    ...
                    `}>
                        <input
                          ...
                          value={searchValue}
                          onChange={(e) => setSearchValue(e.target.value)}
                          onKeyDown={(e) => { // <-- Mobile par bhi ENTER key dabane par search hoga
                            if (e.key === 'Enter') {
                              executeSearch(searchValue);
                            }
                          }}
                        ...
                        <Search 
                          onClick={(e) => { // <-- Mobile par bhi Search Icon click karne par search hoga
                            e.stopPropagation();
                            executeSearch(searchValue);
                          }} 
                          className={`  px-2 rounded-r-full ${dark ? "bg-[#191919] " : "bg-white "} duration-500 `} size={40} />
                      </div>
                    </div>
                  </>
                ) : null
              ...
              {searchHistoryData.map((data, i) => (
                  <div key={i}
                    onClick={(e) => { // <-- Mobile history click karne par search hoga
                      e.stopPropagation();
                      setSearchValue(data);
                      executeSearch(data);
                    }}
          ...
          <div
            ...
            <div
             ...
             ${createOption ? "flex" : "hidden"} flex-col duration-500`}>
              ...
              >
                <YoutubeSVG /> Upload video {/* Yahan add kiya */}
              </div>
              <div
                className={` flex p-2 gap-3 cursor-pointer   text-sm items-center ${dark ? " hover:bg-[#242424] " : "hover:bg-gray-200"} rounded-lg`}
              >
